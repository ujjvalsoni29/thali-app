...
**Because of that, no href, import or fetch in app code may point into `Main/`.** It is not on the Vite root and not in the repo, so anything reaching for it resolves against the URL instead of the disk, falls through to the SPA fallback and renders an empty route. If a file in `Main/` needs to be reachable from the running app, it gets a generated copy in `public/` and a drift check — see `scripts/sync-idea-bank.mjs` and `check.mjs` check 11, which is what the Idea bank link needed.
...
| Nothing in `Main/` may be linked from app code | §10, `check.mjs` 11 | **Thali**, step 12 review. The masthead's Idea bank button pointed at `../Main/thali-idea-bank.html` — correct under `file://`, and under the Worker it collapsed to `/Main/thali-idea-bank.html`, hit the SPA fallback and rendered a blank page, because `App.tsx` has two routes and no catch-all. `Main/` is outside the Vite root AND outside version control, so it can never be served. Manc found it by clicking it: *"why does my idea bank still link to an html page that does not even render anything?"*. The served copy now lives in `public/`, generated by `scripts/sync-idea-bank.mjs` |
